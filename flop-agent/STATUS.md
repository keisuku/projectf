...
| close-1 (Close Call) | **READ 2026-09-26; recommendation: register only, do not trade — awaiting the operator's yes** | `flop-labs/technocore-close-call-challenge` @ `66c1da3`, `rules_version 0.1-draft`. Bet on the last Hyperliquid `xyz:NVDA` trade before 2026-10-04T10:00Z; 10,000 POLF per owner key; top three share 1,000,000 FLOP after mainnet. **Identity is not checked** (rule 8, `identity_policy: any did:key`), so the gate that excluded us from sonnet-2 does not exist here — and the same rule lets one operator run many keys, which fills the top three with key-farms betting max size both ways. We run one key and will not mint another (HANDOFF §2). Fee rule (§12): the side that got a *better* price than the sweep's close pays the gap back, so price-squeezing a counterparty earns nothing; score ≈ size × direction − fees. For one key that is a coin-flip on NVDA with no prize expectancy. **Registering alone is near-free and still worth it:** one signed post in the public `close1` room, and rule 17 ranks every owner however little it traded, so the DID lands in a referee-read, archived ledger — the crawlability sonnet-2 showed we lacked. Body (97 chars, identity under upstream `clean_text`, sha256 `80fb0c5f…92be`): `{"t":"owner","season":"close-1","key":"did:key:z6MkhCvnKQ9E9eZxK7wcS2FJ1Diir2rgfTkaYbMnczha9QDU"}`. **Open gap:** the package says it is a draft until FLOP Labs signs a launch record, and no referee DID is published in the repo; nothing read from `d-close1-*` is trusted until one is. Registration closes at the lock, 2026-10-04T09:00Z. |
